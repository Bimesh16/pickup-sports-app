...
public class MfaController {
    ...
    public ResponseEntity<?> enable(Principal principal, @RequestBody Map<String, Object> body) {
        ...
        if (!mfa.verify(principal.getName(), code)) {
            return ResponseEntity.status(400).headers(noStore()).body(new com.bmessi.pickupsportsapp.dto.api.MessageResponse("invalid_code"));
        }
        mfa.enable(principal.getName());
        return ResponseEntity.ok().headers(noStore()).body(new com.bmessi.pickupsportsapp.dto.api.EnabledResponse(true));
    }
    ...
    public ResponseEntity<com.bmessi.pickupsportsapp.dto.api.EnabledResponse> disable(Principal principal) {
        ...
        return ResponseEntity.ok().headers(noStore()).body(new com.bmessi.pickupsportsapp.dto.api.EnabledResponse(false));
    }
    ...
    public ResponseEntity<com.bmessi.pickupsportsapp.dto.api.ValidResponse> verify(Principal principal, @RequestBody Map<String, Object> body) {
        ...
        return ResponseEntity.ok().headers(noStore()).body(new com.bmessi.pickupsportsapp.dto.api.ValidResponse(ok));
    }
